...
EOT;
        $technologies = ['html5', 'css', 'jquery', 'php', 'symfony'];
        $shortDesc = "Annonces immobilières";
        $imgPath = '/uploads/images/projects/immo_express_1.jpg';
        $category = "project-vitrine";
        $this->loadBasic($title, $description, $shortDesc, $technicalPoints, $imgPath, $category, $technologies);
    }
...
EOT;
        ...
        $category = "project-vitrine";
        $this->loadBasic($title, $description, $shortDesc, $technicalPoints, $imgPath, $category);
    }
...
EOT;
    ...
    protected function loadStudioForLms()
    {
        ...
        $description = <<<EOT
Extranet des clients de Momindum Maker, Maker Lite (Application de création de contenu Rich Media).
...
EOT;
    ...
    protected function loadMakerLite()
    {
        $title = 'Maker Lite';
        $description = <<<EOT
SAAS permet de créer du contenu Rich media. Le produit final est une vidéo synchronisée
...
EOT;
    ...
    {
        $title = 'Allopneus site mobile';
        $description = <<<EOT
Site mobile E-commerce de vente de pneus.
...
EOT;
    ...
    protected function loadAllopneus()
    {
        $title = 'Allopneus';
        $description = <<<EOT
Site E-commerce leader dans la vente de pneus. Refonte de la partie jante du site desktop.
...
EOT;
    ...
    protected function loadBasic($title, $description, $shortDescription, $technicalPoints, $imgPath, $category, $technologies = [])
    {
        $project = new Project();
        ...
        $project->setTechnicalPoints($technicalPoints);

        foreach ($technologies as $tech) {
            $project->addWebTechnology($this->getReference($tech));
        }
